...
import {
  getComparePendingImport,
  getImportedList,
  hasImportedList,
  resetComparePendingImport,
  setComparePendingImport,
  setImportedList,
...
/**
 * @brief Handles the import fetch button click — validates key, fetches list, opens name modal.
 *
 * @return {Promise<void>}
 */
async function handleImportFetch() {
  ...
  try {
    ...
    setComparePendingImport({ key, ...parsed });

    // Close import modal, open name modal
    document.getElementById('importBg').style.display = 'none';

    const now = new Date();
    const defaultName =
      'Imported List ' + now.toLocaleString('en', { month: 'long', year: 'numeric' });
    const existingList = getImportedList(key);
    document.getElementById('importNameInput').value = existingList?.name || defaultName;

    // Show duplicate warning if this key already exists
    const dupWarn = document.getElementById('importDupWarning');
    const dupSpan = document.getElementById('importDupName');
    if (hasImportedList(key)) {
      dupWarn.style.display = '';
      dupSpan.textContent = getImportedList(key).name;
    } else {
...
export function initShare() {
  ...
  document.getElementById('importFetchBtn').addEventListener('click', handleImportFetch);
  ...
  document.getElementById('importSaveBtn').addEventListener('click', () => {
    ...
    const pending = getComparePendingImport();
    if (!name || !pending) return;
    const { key, labels, marks } = pending;
    setImportedList(key, {
      key,
      name,
