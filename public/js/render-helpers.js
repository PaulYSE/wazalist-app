/* render-helpers.js — small pure-ish helpers shared by the renderers:
   marking styles/pips and video/oEmbed handling. */
    ...
    function markingStyle(markings) {
    ...
    const markingPips = markings => SHAPES.map((s, i) => '<span class="marking-pip' + (markings[i] ? ' on' : '') + '" title="' + (markingLabels[i] || 'Marking ' + (i + 1)) + '">' + s + '</span>').join('');
